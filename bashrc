...
function gitpushbranch()
{
    ...
    GIT_PREVIOUS_BRANCH=$(git rev-parse --abbrev-ref HEAD)
    export GIT_PREVIOUS_BRANCH
    git checkout $1
}

function gitpopbranch()
{
    if  [ "x${GIT_PREVIOUS_BRANCH}" == "x"  ]
    then
        echo "GIT_PREVIOUS_BRANCH is null"
        return
    fi
    tmp_GIT_PREVIOUS_BRANCH=${GIT_PREVIOUS_BRANCH}
    GIT_PREVIOUS_BRANCH=$(git rev-parse --abbrev-ref HEAD)
    export GIT_PREVIOUS_BRANCH
    git checkout ${tmp_GIT_PREVIOUS_BRANCH}
}
...
# sed -n '/^ngx_module_t/{ :front; p; n; /};/!{b front}; p; }' event/ngx_event.c
